...
export function validateConfig(config: JarbasConfig): void {
  if (!loopbackHosts.has(config.server.host)) {
    throw new Error("JARBAS_SERVER_HOST must be a loopback address in Foundation V1");
  }
  ...
  if (!runtimeIds.has(config.runtime.defaultProviderId)) {
    throw new Error(`Unknown default runtime: ${config.runtime.defaultProviderId}`);
  }
  ...
  for (const runtime of config.runtimes) {
    ...
    if (endpoint.username || endpoint.password) {
      throw new Error(`Runtime ${runtime.id} endpoint must not contain credentials`);
    }
    if (runtime.local && !loopbackHosts.has(endpoint.hostname)) {
      throw new Error(`Local runtime ${runtime.id} must use a loopback endpoint`);
    }
  }
  ...
  const preset = config.presets.find((candidate) => candidate.id === config.routing.preset);
  if (!preset) {
  ...
  for (const modelId of Object.values(preset.routes)) {
    if (!modelIds.has(modelId)) {
      throw new Error(`Preset ${preset.id} references unknown model ${modelId}`);
    }
  }
